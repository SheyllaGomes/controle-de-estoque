    ...
    <article class="card" id="print-area">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${report.rows.length ? report.rows.map((row) => `<tr>${row.map((cell) => `<td>${escapeHtml(String(cell))}</td>`).join("")}</tr>`).join("") : `<tr><td colspan="${report.headers.length}">Nenhum registro encontrado.</td></tr>`}
          </tbody>
        </table>
      </div>
